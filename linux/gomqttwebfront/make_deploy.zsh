...
go build "$@"  && rsync ${RSYNCOPTIONS[@]} -rvp --delay-updates --progress --delete ${PWD:t} config.env public ${REMOTE_USER}@${REMOTE_HOST}:${REMOTE_DIR}/ && echo "please run systemctl restart gomqttwebfront.service as root on licht"
