...
const (
	DEFAULT_GONAMECTRL_MQTTBROKER   string = "tcp://mqtt.realraum.at:1883"
	DEFAULT_GONAMECTRL_RF433TTYDEV  string = "/dev/ttyACM0"
	DEFAULT_GONAMECTRL_BUTTONTTYDEV string = "/dev/ttyACM1"
)
...
func goConnectToMQTTBrokerAndFunctionWithoutInTheMeantime(tty_rf433_chan chan SerialLine) {
	...
	for {
		mqttc := ConnectMQTTBroker(EnvironOrDefault("GONAMECTRL_MQTTBROKER", DEFAULT_GONAMECTRL_MQTTBROKER), EnvironOrDefault("GONAMECTRL_CLIENTID", r3events.CLIENTID_LIGHTCTRL))
...
func main() {
	...
	if UseFakeGPIO_ {
		...
		tty_rf433_chan, _, err = OpenAndHandleSerial(EnvironOrDefault("GONAMECTRL_RF433TTYDEV", DEFAULT_GONAMECTRL_RF433TTYDEV), 9600)
		if err != nil {
			panic("can't open GONAMECTRL_RF433TTYDEV")
		}

		_, tty_button_chan, err = OpenAndHandleSerial(EnvironOrDefault("GONAMECTRL_BUTTONTTYDEV", DEFAULT_GONAMECTRL_BUTTONTTYDEV), 9600)
		if err != nil {
			panic("can't open GONAMECTRL_BUTTONTTYDEV")
		}
	}
